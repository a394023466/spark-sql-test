...
object MtaLogConnMain {
  /*def main(args: Array[String]): Unit = {
    val HDFS_FILE_INPUT_PATH = "hdfs://hadoop001:8020/opt/testfile/mta.log.20150528.14328288010.74891400"
    val HDFS_FILE_OUTPUT_PATH = "hdfs://hadoop001:8020/opt/testfile/output/conn"

    val spark = SparkSession.builder().appName("MtaLog").getOrCreate()

    val connMsg = spark.sparkContext.textFile(HDFS_FILE_INPUT_PATH)
    val messageRDD: RDD[Row] = MatlogPattern.getConnectionRowRDD(connMsg)
    ...
    val df = spark.createDataFrame(messageRDD, MatlogStructType.getConnectionStructType)
